...
//        session.sendMessage(textMessage);
        ...
        Room room = roomService.findRoomById(chatMessage.getRoom().getId()); // room 은 포스트맨으로 미리 만들어놓고 다시 해보면 됨 오늘은 여기까지 쉬버
        log.info("room {}", room.toString());

        chattingService.handleAction(session, chatMessage); // 여기부터 계속하면 되겠다
    }
}